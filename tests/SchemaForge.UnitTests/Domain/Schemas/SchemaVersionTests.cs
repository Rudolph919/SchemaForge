...
{
    private static SchemaVersion NewDraft() =>
        SchemaVersion.CreateDraft(Guid.NewGuid(), Guid.NewGuid(), SemVer.Initial);
    ...
    public void CreateDraft_defaults_to_draft_status_with_an_empty_object_root_and_raises_an_event()
    {
        ...
        version.DomainEvents.Should().ContainSingle().Which.Should().BeOfType<SchemaVersionCreated>();
    }
    ...
    public void Publish_transitions_draft_to_published_and_raises_an_event()
    {
        ...
        version.DomainEvents.Should().Contain(e => e is SchemaVersionPublished);
    }
    ...
    public void Deprecate_transitions_published_to_deprecated_and_raises_an_event()
    {
        ...
        version.DomainEvents.Should().Contain(e => e is SchemaVersionDeprecated);
    }
    ...
    public void AddObjectProperty_adds_a_child_to_the_root_and_raises_an_event()
    {
        ...
        var result = version.AddObjectProperty(version.RootNode.Id, "invoiceNumber", NodeKind.String);

        result.IsSuccess.Should().BeTrue();
        version.RootNode.Properties.Should().ContainSingle(n => n.Id == result.Value && n.PropertyName == "invoiceNumber");
        version.DomainEvents.Should().Contain(e => e is SchemaNodeAdded);
    }
    ...
    public void AddObjectProperty_fails_when_the_parent_node_does_not_exist()
    {
        ...
        var result = version.AddObjectProperty(Guid.NewGuid(), "invoiceNumber", NodeKind.String);
    ...
    {
        var version = NewDraft();
        var arrayNodeId = version.AddObjectProperty(version.RootNode.Id, "lineItems", NodeKind.Array).Value;
    ...
    public void UpdateNode_applies_new_content_and_raises_an_event()
    {
        ...
        var content = SchemaNodeContent.Empty(NodeKind.String) with
        {
            ...
            StringConstraints = new(null, 255, null, SchemaForge.Domain.Schemas.SchemaFormat.Email, null),
        };
        var result = version.UpdateNode(nodeId, content);

        result.IsSuccess.Should().BeTrue();
        var node = version.RootNode.Properties.Single(n => n.Id == nodeId);
        node.Description.Should().Be("The customer's email address");
        node.IsRequiredByParent.Should().BeTrue();
        node.StringConstraints!.Format.Should().Be(SchemaForge.Domain.Schemas.SchemaFormat.Email);
    ...
    public void RemoveNode_removes_a_nested_property_and_raises_an_event()
    {
        ...
        result.IsSuccess.Should().BeTrue();
        version.RootNode.Properties.Should().BeEmpty();
        version.DomainEvents.Should().Contain(e => e is SchemaNodeRemoved);
    }
