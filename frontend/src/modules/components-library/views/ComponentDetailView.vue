...
import type { VersionBumpKind } from '@/types/schemas'

const route = useRoute()
const router = useRouter()
const componentId = computed(() => route.params.componentId as string)

const component = ref<ComponentDefinitionDetailResponse | null>(null)
const versions = ref<ComponentVersionSummaryResponse[]>([])
...
async function load() {
  ...
  try {
    const [componentResponse, versionsResponse] = await Promise.all([
      componentsApi.getComponent(componentId.value),
      componentVersionsApi.listVersions(componentId.value),
    ])
    component.value = componentResponse
    versions.value = versionsResponse
  } catch (error) {
    loadError.value = error instanceof ApiError ? error.message : 'Could not load component.'
  }
}

function startEditing() {
  if (!component.value) return
  editName.value = component.value.name
  editDescription.value = component.value.description ?? ''
...
async function handleSaveDetails() {
  ...
  isSaving.value = true
  try {
    await componentsApi.updateComponentDetails(componentId.value, {
      name: editName.value,
...
function statusClass(status: ComponentVersionSummaryResponse['status']): string {
  switch (status) {
    case 'Draft':
...
<template>
  <div>
    <button type="button" class="text-sm text-slate-500 hover:text-slate-700" @click="router.push('/components')">
      ← Back to Components
    </button>
    ...
    <template v-if="component">
      <div class="mt-4 rounded-lg border border-slate-200 bg-white p-6">
        <div v-if="!isEditing" class="flex items-start justify-between">
          <div>
            <h1 class="text-lg font-semibold text-slate-900">{{ component.name }}</h1>
          ...
          <tbody>
            <tr v-for="version in versions" :key="version.id" class="border-b border-slate-100 last:border-0">
              ...
              <td class="space-x-3 py-2 text-right">
                <router-link :to="`/component-versions/${version.id}`" class="text-slate-500 hover:text-slate-900">
