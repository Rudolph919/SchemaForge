...
2. **The integration seam is already asynchronous.** Step 1 §8 and Step 6 §4 already route test runs through the outbox/background-worker pattern, returning `202 Accepted` and polling for status. That's *already* a queue-shaped boundary, not a synchronous in-process call — extracting the worker into its own deployable means swapping the in-process `BackgroundService` for a real consumer of the same outbox contract, which Step 1 §8 explicitly designed the `IJobDispatcher` port to make possible without touching any caller.
